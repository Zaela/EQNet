...
uint32_t updateCRC(byte* data, int len, uint32_t crc = 0xFFFFFFFF)
{
	for (int i = 0; i < len; ++i)
...
namespace CRC
{
	uint16_t calc(void* data, int len, uint32_t key)
	{
		...
		uint32_t crc = updateCRC(keybytes, sizeof(uint32_t));
	...
	bool validatePacket(void* packet, uint32_t& len, uint32_t key)
	{
		...
		uint16_t checkCRC = toHostShort(*(uint16_t*)(data + len));
		if (checkCRC == 0)
			return true;
